...
from utils.common_func import read_yaml, load_data
...
class ModelTraining:
    ...
    def train_lgbm(self,X_train, y_train):
        try:
            logger.info("Initialising the lgbm model")
            ...
            logger.info(f"Error while training the data : {e}")

            raise CustomException("Faile to train model", e)
        
    
    def evaluste_model(self, model, X_test, y_test):
        try:
            ...
            logger.info(f"precison : {precision}")
    ...
    def run(self):
        try:
            with mlflow.start_run():
                ...
                logger.info("Starting out MLFLOW experimentation")
                ...
                metrics = self.evaluste_model(best_lgbm_model, X_test,y_test)
                ...
                logger.info("Saving the params and metrics to MLFLOW")
